...
public class RefuseOnFoodServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            request.setAttribute("message", "该待审核商品不存在");
            e.printStackTrace();
            request.getRequestDispatcher("OnFood").forward(request,response);
            return;
        }
        if(foodId!=0 && fs.refuseOnFoodService(foodId)){
            request.setAttribute("message","审核不通过成功");
            request.getRequestDispatcher("OnFood").forward(request,response);
        }else {
            request.setAttribute("message","审核不通过失败");
            request.getRequestDispatcher("OnFood").forward(request,response);
        }
    }
}
